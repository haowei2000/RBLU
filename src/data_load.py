...
def load_qa(
    ...
    dataset = rename(dataset, ["input", "question", "body", "selftext", "instruction"], "question")
    ...
    elif count is not None and len(dataset) > count:
...
if __name__ == "__main__":
    for task in [
        ...
        for language in ["zh", "en"]:
            result = load_qa(language, task, 10, 1000, None, True)
            print(f"Language: {language}, Task: {task}, Count: {len(result[0])}")
