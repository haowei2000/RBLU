...
def load_field(
    field: str, count: int, min_length: int = 50, max_length: int = 100
) -> pd.DataFrame:
    """
    ...
    if field == "code":
        ...
        records = [
            ...
            if min_length < len(record["question"]) < max_length
        ]
...
def plot_string_length_distribution(data: pd.DataFrame, key: str) -> None:
    ...
    histogram = data[key + "_length"].plot.hist(
        figsize=(10, 6), bins=20, range=(0, 200)
    )
