...
def plot_ellipse(fig, rotated_ellipse, color, alpha=0.5, name=None):
...
def _scatter_3D(round, doc_count, vector, colors, fig):
    ...
    if vector is not None:
        for i in range(round):
            ...
            fig.add_trace(
                go.Scatter3d(
                    x=vector["x"][start_idx:end_idx],
                    y=vector["y"][start_idx:end_idx],
                    z=vector["z"][start_idx:end_idx],
                    mode="markers",
                    marker=dict(size=5, color=colors[i], opacity=1 - i * 0.1),
                    name=f"Round {i}",
                )
            )
            ...
            plot_ellipse(
                ...
                rotated_ellipse,
                color=colors[i],
                name=f"Ellipse Round {i}" if i == 0 else "",
            )
    else:
...
def draw_tsne_single(
    ...
    for target, language in product(["q", "a"], language_list):
        for model_name, language, task in product(
            ...
            fig.update_layout(
                title=(
                    f"{translate_model(model_name)} - "
                    f"{task.capitalize()}-{translate_language(language).capitalize()}"
                ),
                scene=dict(
                    xaxis_title="X",
                    yaxis_title="Y",
            ...
            output_path = output_dir / f"{model_name}_{task}.{suffix}"
